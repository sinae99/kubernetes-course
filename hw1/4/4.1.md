...
Systemd manages all processes and system resources by using **Control Groups (cgroups)**

a Linux kernel feature that organizes everything into a hierarchical tree for resource allocation.

The three unit types responsible for this organization are Services, Slices, and Scopes.

---

### 1. Service vs. Daemon: Defining the Workload

The concepts of the Service unit and the daemon process are related by control and encapsulation:

*   **Daemon (The Process):** This is the executable program running in the background (e.g., a web server). This is the actual workload assigned a PID.
...
In the cgroup hierarchy, only Services and Scopes are the "leaf nodes" that contain actual running processes.
...
| **Creation** | Scopes are **dynamically created** using systemd’s API, rather than static unit files. |
